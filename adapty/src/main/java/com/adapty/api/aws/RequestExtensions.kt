...
private fun Request.amazonSecurityHeader() =
    header("x-amz-security-token")
        ?: throw NoSuchFieldException("Request cannot be signed without having the x-amz-security-token header")

private fun Request.amazonTargetHeader() =
    header("x-amz-target")
        ?: throw NoSuchFieldException("Request cannot be signed without having the x-amz-target header")
